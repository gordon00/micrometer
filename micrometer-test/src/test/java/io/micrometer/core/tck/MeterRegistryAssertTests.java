...
class MeterRegistryAssertTests {
    ...
    void assertionErrorThrownWhenTimerPresentWithCommonTagValue() {
        ...
        assertThatThrownBy(() -> meterRegistryAssert.doesNotHaveTimerWithNameAndTags("matching-metric-name", Pairs.of("matching-tag", "matching-value")))
                .isInstanceOf(AssertionError.class)
                .hasMessageContaining("Expected no timer with name <matching-metric-name> and tags <[tag(matching-tag=matching-value)]> but found one");
    }
    ...
    void noAssertionErrorThrownWhenTimerWithCommonTagPresent() {
        ...
        assertThatCode(() -> meterRegistryAssert.hasTimerWithNameAndTags("matching-metric-name", Pairs.of("matching-tag", "matching-value")))
            .doesNotThrowAnyException();
    }
    ...
    void noAssertionErrorThrownWhenTimerWithCommonTagsMissing() {
        assertThatCode(() -> meterRegistryAssert.doesNotHaveTimerWithNameAndTags("foo", Pairs.of(Pair.of("bar", "baz"))))
                .doesNotThrowAnyException();
    }
